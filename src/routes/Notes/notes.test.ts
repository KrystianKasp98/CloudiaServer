...
describe('/notes route [SUCCESS]', () => {
  ...
  it('[GET] /:id', async () => {
    const res: ExpressResult = await request(app).get(
      `${PATHS.NOTES}/63b4b2da1d7d5633f541f28c`
    );
  ...
  it('[POST], [PUT], [DELETE] /', async () => {
    const inputPostBody = {
      note: 'I <3 CloudiaServer',
      date: '2022-12-26T22:19:56.945Z'
    };
    ...
    const resPut: ExpressResult = await request(app)
      .put(`${PATHS.NOTES}/${noteId}`)
      .send(inputPutBody);
...
describe('/notes route [FAIL]', () => {
  ...
  it('[PUT] /:id, empty note', async () => {
    const res: ExpressResult = await request(app)
      .put(`${PATHS.NOTES}/63a24fa71e33c4ef5943a37b`)
      .send({ note: '' });
  ...
  it('[DELETE] /:id, bad id', async () => {
    const res: ExpressResult = await request(app).delete(
      `${PATHS.NOTES}/wrongid`
    );
